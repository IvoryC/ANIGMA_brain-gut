...
#' 
## ----plotDiv--------------------------------------------------------------------------------------------------------------
sizeByP <- function(p, sig=0.05){
  size = ifelse(p < sig, (-log(p)-1), 1)
  return(size)
}

plotDiv = function(df, column="diversity", sig=0.05, saveToDir=outDir, tLevel=taxaLevel){
  ...
  if (!is.na(sig)){
    ...
    ysigs = c(HCvT1p=0.9, HCvT2p=.95, T2vT1p=1)
    ysigs = ysigs * 3
