...
We have previously calculated differences:
```{r}
...
TaxaVsCategoryTest <- function(dataDF, taxa, variables, doPlot=TRUE, redPval=0.01, testType="anova", fileNameBase=NULL, minGroupSize=2){
    ...
    pvalCountTable = data.frame(variable="TOTAL",
                                ...
                                nVals=NA,
                                levels=NA,
                                attemptedTests=length(taxa) * length(variables),
                                testWithPvalue=NA,
                                rawPvalSig=NA,
                                adjPvalSig_nTaxa=NA,
    ...
    for (variable in variables){
        ...
        # adjust based on all tests this is run of the function
        moreAdjustedPvals = p.adjust(variablePvals, method = padj.method, 
                                     n=length(taxa) * length(variables))
        ...
        pvalCountTable = rbind(pvalCountTable, 
                               c(
                                   ...
                                   attemptedTests=length(taxa),
                                   testWithPvalue=sum(!is.na(variablePvals)),
                                   rawPvalSig=sum(variablePvals <= redPval, na.rm=T),
                                   adjPvalSig_nTaxa=sum(adjustedPvals <= redPval, na.rm=T),
                                   adjPvalSig_nTests=sum(moreAdjustedPvals <= redPval, na.rm=T)
                               ) 
        )
    ...
    pvalCountTable[1,"testWithPvalue"] = sum(!is.na(pvalTable))
    pvalCountTable[1,"rawPvalSig"] = sum(pvalTable <= redPval, na.rm=T)
    pvalCountTable[1,"adjPvalSig_nTests"] = sum(p.adjust(unlist(pvalTable), method = padj.method) <= redPval, na.rm=T)
    
    
    # save p-value table
    if (saveToFile){
        table.file = paste0(fileNameBase, "_p-values.txt")
        message("Saving file: ", table.file)
        write.table(file=table.file, pvalTable, sep="\t", quote=F, row.names = F)
    }else{
        pvalCountTable
    }
    
    # show or save summary table
    if (saveToFile){
...
anovaAtT1 = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T2"), 
                               ...
                               fileNameBase=file.path(outDir, "HCT1_variables_byTime"))
anovaAtT2 = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T1"), 
                               taxa, variables_byTime, 
                               fileNameBase=file.path(outDir, "HCT2_variables_byTime"))
anovaAtHC = TaxaVsCategoryTest(data %>% filter(TIMEPOINT == "HC"), 
                               taxa, "LOCATION", 
                               fileNameBase=file.path(outDir, "HC_Location"))

# nonparametric test
kwAcrossTime = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "HC"), taxa, variables_acrossTime, doPlot=F, testType = "kruskal.test")
# randomized.kwAcrossTime = TaxaVsCategoryTest(scrambled.data, taxa, variables_acrossTime, doPlot=F, testType = "kruskal.test")
kwAtT1 = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T2"), taxa, variables_byTime, doPlot=F, testType = "kruskal.test")
kwAtT2 = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T1"), taxa, variables_byTime, doPlot=F, testType = "kruskal.test")
kwAtHC = TaxaVsCategoryTest(data %>% filter(TIMEPOINT == "HC"), taxa, "LOCATION", doPlot=F, testType = "kruskal.test")
```
...
#         pdfName = file.path(outDir, paste0("pval-hist-", title,"-", taxalevel, ".pdf"))
#         message(pdfName)
...
# suppressWarnings({
#     suppressMessages({
#         pvalHist(acrossTimeTable, "ANOVA-acrossTime")
#         pvalHist(anovaAtT1, "ANOVA-T1")
#         pvalHist(anovaAtT2, "ANOVA-T2")
#         pvalHist(anovaAtHC, "ANOVA-HC")
#         
#         pvalHist(kwAcrossTime, "Kruskal-Wallis-acrossTime")
#         pvalHist(kwAtT1, "Kruskal-Wallis-T1")
#         pvalHist(kwAtT2, "Kruskal-Wallis-T2")
#         pvalHist(kwAtHC, "Kruskal-Wallis-HC")
#     })
# })
```
...
<!-- # heatmap -->
<!-- # plot for file -->
<!-- pvalHeatMap.categories.file = file.path(outDir, paste0("pval-heatmap-categories-", taxaLevel, ".pdf")) -->
<!-- pdf(file=pvalHeatMap.categories.file, -->
<!--     height = 3 + nrow(m.all.1) / 12, -->
<!--     width = 5 + ncol(m.all.1) / 6) -->
<!-- pvalTableToHeatMap(m.all.1, title="catogrical-features") -->
<!-- dev.off() -->
<!-- ``` -->

<!-- See file ``r pvalHeatMap.categories.file``. -->
...
```{r TaxaVsNumericTest}
TaxaVsNumericTest <- function(dataDF, taxa, variables, doPlot=TRUE, redPval=0.01, testType="pearson"){
...
suppressWarnings({
    suppressMessages({
        peAtT1 = TaxaVsNumericTest(data %>% filter(TIMEPOINT == "T1"), taxa, diffables, doPlot=F, testType = "pearson")
        keAtT1 = TaxaVsNumericTest(data %>% filter(TIMEPOINT == "T1"), taxa, diffables, doPlot=F, testType = "kendall")
        peAtT2 = TaxaVsNumericTest(data %>% filter(TIMEPOINT == "T2"), taxa, diffables, doPlot=F, testType = "pearson")
        keAtT2 = TaxaVsNumericTest(data %>% filter(TIMEPOINT == "T2"), taxa, diffables, doPlot=F, testType = "kendall")
...
# suppressWarnings({
#     suppressMessages({
#         pvalHist(peAtT1, "Pearson-T1")
#         pvalHist(keAtT1, "kendall-T1")
#         pvalHist(peAtT2, "Pearson-T2")
#         pvalHist(keAtT2, "kendall-T2")
#         pvalHist(peAtDif1, "Pearson-diff")
#         pvalHist(keAtDif1, "kendall-diff")
#     })
# })
```
...
```{r pvalHeat}
pvalTableToHeatMap = function(pvalTable, midpointPval=0.05, bluePvalue=0.05, redPvalue=0.00001, title="-log10(p-values)", displayName.FUN=displayTaxonName){
    ...
    longTab = -log10(pvalTable) %>% 
        mutate(taxon= sapply( row.names(pvalTable), displayName.FUN) ) %>%
...
```{r}
# n.a1 = pvalTableToHeatMap(peAtT1, title="Pearson-T1")
# n.a2 = pvalTableToHeatMap(keAtT1, title="kendall-T1")
# n.b1 = pvalTableToHeatMap(peAtT2, title="Pearson-T2")
# n.b2 = pvalTableToHeatMap(keAtT2, title="kendall-T2")
# n.c1 = pvalTableToHeatMap(peAtDif1, title="Pearson-diff-T1bugs")
# n.c2 = pvalTableToHeatMap(keAtDif1, title="kendall-diff-T1bugs")
# n.d1 = pvalTableToHeatMap(peAtDif2, title="Pearson-diff-T2bugs")
# n.d2 = pvalTableToHeatMap(keAtDif2, title="kendall-diff-T2bugs")
# 
# 
# 
# ggarrange(n.a1, n.a2, align="h", common.legend = T)
# ggarrange(n.b1, n.b2, align="h", common.legend = T)
# ggarrange(n.c1, n.c2, align="h", common.legend = T)
```

```{r}
# ## Save ALL the heat-maps to a file.
# 
# pvalHeatMap.file = file.path(outDir, paste0("pval-heatmaps-", taxaLevel, ".pdf"))
# pdf(file=pvalHeatMap.file,
#     height = 2 + length(taxa) / 4,
#     width = 5)
# a1
# a2
# b1
...
## Heatmaps were saved to file: ``r pvalHeatMap.file``
```
...
# heatmap
# plot for file
pvalHeatMap.numeric.file = file.path(outDir, paste0("pval-heatmap-numeric-", taxaLevel, ".pdf"))
pdf(file=pvalHeatMap.numeric.file,
    height = 3 + nrow(m.all.2) / 12,
    width = 5 + ncol(m.all.2) / 6)
pvalTableToHeatMap(m.all.2, title="numeric-features")
dev.off()
```


Merge the two heatmaps: the big heatmap
```{r}
pvalHeatMap.categories.file = file.path(outDir, paste0("pval-heatmap-all-", taxaLevel, ".pdf"))
pdf(file=pvalHeatMap.categories.file,
    height = 3 + nrow(m.all) / 12,
    width = 5 + ncol(m.all) / 6)
pvalTableToHeatMap(m.all, title="the big heatmap")
dev.off()
```
...
Draw pvalue histograms for all variables.
```{r}
pvalHist = function(pvalTable, title="", pdfName=title, saveToFolder=outDir, taxalevel=taxaLevel){
    longTab = cbind(taxon=row.names(pvalTable), pvalTable) %>% 
        pivot_longer(cols=-taxon, names_to = "variable", values_to = "value")
