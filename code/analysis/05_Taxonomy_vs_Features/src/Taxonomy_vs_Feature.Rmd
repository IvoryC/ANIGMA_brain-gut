...
write.table(data.frame(shortName=shortName, 
...
TaxaVsCategoryTest <- function(dataDF, taxa, variables, doPlot=TRUE, redPval=0.01, testType="anova", fileNameBase=NULL, minGroupSize=2){
    ...
    if (saveToFile){
        ...
        write.table(file=table.file, 
                    cbind(taxa=row.names(pvalTable), pvalTable), 
                    sep="\t", quote=F, row.names = F)
        
        # save adj p-value table
        table.file = paste0(fileNameBase, "_p-values_adjustedForNtaxa.txt")
        message("Saving file: ", table.file)
        write.table(file=table.file, 
                    cbind(taxa=row.names(adjPvalTable), adjPvalTable), 
                    sep="\t", quote=F, row.names = F)
    ...
    return(list(p.values=pvalTable, adjPvals=adjPvalTable, summary=pvalCountTable))
}
```

```{r runTaxaVsCategoryTest}
# parametric test - anova
acrossTimeTableList = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "HC"), 
                                     taxa, variables_acrossTime, 
                                     fileNameBase=file.path(outDir, "T1T2.anova"))
acrossTimeTable = acrossTimeTableList$p.values

# randomized.acrossTimeTable = TaxaVsCategoryTest(scrambled.data, taxa, variables_acrossTime, doPlot=F)

anovaAtT1List = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T2"), 
                               taxa, variables_byTime, 
                               fileNameBase=file.path(outDir, "HCT1_variables_byTime"))
anovaAtT1 = anovaAtT1List$p.values

anovaAtT2List = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T1"), 
                               taxa, variables_byTime, 
                               fileNameBase=file.path(outDir, "HCT2_variables_byTime"))
anovaAtT2 = anovaAtT2List$p.values

anovaAtHCList = TaxaVsCategoryTest(data %>% filter(TIMEPOINT == "HC"), 
                               taxa, "LOCATION", 
                               fileNameBase=file.path(outDir, "HC_Location"))
anovaAtHC = anovaAtHCList$p.values

# nonparametric test
kwAcrossTime = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "HC"), 
                                  taxa, variables_acrossTime, doPlot=F, testType = "kruskal.test")$p.values
...
kwAtT1 = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T2"), 
                            taxa, variables_byTime, doPlot=F, testType = "kruskal.test")$p.values
kwAtT2 = TaxaVsCategoryTest(data %>% filter(TIMEPOINT != "T1"), 
                            taxa, variables_byTime, doPlot=F, testType = "kruskal.test")$p.values
kwAtHC = TaxaVsCategoryTest(data %>% filter(TIMEPOINT == "HC"), 
                            taxa, "LOCATION", doPlot=F, testType = "kruskal.test")$p.values
```
...
pvalTableToHeatMap(m.all.2, title="numeric-features", displayName.FUN=function(input){return(input)})
dev.off()
```
...
```{r}
pvalHeatMap.categories.file = file.path(outDir, paste0("bigHeatmap_", taxaLevel, ".pdf"))
pdf(file=pvalHeatMap.categories.file,
...
As a convenience, stack all of the summary tables that we made along the way into one table.
```{r}
summaryFiles = dir(path=outDir, pattern="summary.txt", full.names = T)
summaries = lapply(summaryFiles, function(file){
    df = read.delim(file)
    df$testSet = file
    df = df[df$variable != "TOTAL", ]
    return(df)
})
bigSummary = do.call(rbind, summaries)

write.table(bigSummary, file=file.path(outDir, "bigSummary.txt"), quote=F, row.names=F, sep="\t")
```
