...
#' 
#' Pick a single site. Take an argument if one is given.
## -------------------------------------------------------------------------------------------------------------------------
...
taxaLevel = "DiversityMetrics"

#' 
#' Direct output
## -------------------------------------------------------------------------------------------------------------------------
outDir = file.path("..", "output", ONE_SITE)
...
#' ## Read diversity data
#' 
#' Read the diversity data for all levels and merge.
#' 
## -------------------------------------------------------------------------------------------------------------------------
pipeDir="../.."
...
inputFiles=file.path(inputMod, "output", names(inputFileNames), inputFileNames)
names(inputFiles) = names(inputFileNames)
inputFiles = inputFiles[file.exists(inputFiles)]

#' 
## -------------------------------------------------------------------------------------------------------------------------
# divdataList = lapply(inputFiles, read.delim2)
divdataList = list()
for (level in names(inputFiles)){
    basicdf = read.delim2(inputFiles[level]) %>% 
        select(DNA.ID, shannon.diversity, richness)
    basicdf$shannon.diversity = as.numeric(basicdf$shannon.diversity)
    names(basicdf) = c("DNA.ID", paste0(level, ".shannon.diversity"), paste0(level, ".richness"))
    divdataList[[level]] = basicdf
}

if (length(divdataList) > 1){
    divdata = divdataList[[1]]
    for (i in 2:length(divdataList)){
        divdata = merge(divdata, divdataList[[i]], by="DNA.ID")
    }
}else{
    divdata = divdataList[[1]]
}
...
#' 
#' Limit the whole dataset to exactly one site.
## -------------------------------------------------------------------------------------------------------------------------
data = data %>% filter(LOCATION == ONE_SITE)
...
variables_byTime = c("ANOREXIA", "SUBTYPE.AN")
variables_byTime
...
taxa_metrics = names(divdata %>% select(-DNA.ID))
...
TaxaVsCategoryTest <- function(dataDF, taxa_metrics, variables, doPlot=TRUE, redPval=0.05, testType="anova", fileNameBase=NULL, minGroupSize=2, taxaLevel=taxaLevel){
...
write.table(m.all.adj.labeled, file = file.path(outDir, paste0("bigList_pvalues_fdr-adjusted_", ONE_SITE, ".txt")),
...
write.table(summary, file = file.path(outDir, paste0("bigList_summary_", ONE_SITE, ".txt")),
            row.names=F, quote=F, sep="\t")
...
caption.about.names = paste0("Row order here (bottom to top) matches the row order in the corresponding p-value table.
                             This reflects data limited to one site: ", ONE_SITE)
...
## -------------------------------------------------------------------------------------------------------------------------
pvalHeatMap.categories.file = file.path(outDir, paste0("heatmap_categoricalFeatures_raw-p-values_", ONE_SITE, ".pdf"))
...
## -------------------------------------------------------------------------------------------------------------------------
pvalHeatMap.numeric.file = file.path(outDir, paste0("heatmap_numericFeatures_raw-p-values_", ONE_SITE, ".pdf"))
...
## -------------------------------------------------------------------------------------------------------------------------
pvalHeatMap.big.file = file.path(outDir, paste0("bigList_heatmap_raw-p-values_", ONE_SITE, ".pdf"))
...
## -------------------------------------------------------------------------------------------------------------------------
pvalHeatMap.big.adj.file = file.path(outDir, paste0("bigList_heatmap_fdr.adj-p-values_", ONE_SITE, ".pdf"))
